...
export const discoverByBudgetHandler = async (req: Request, res: Response) => {
  try {
    ...
    const allowedSorts: DiscoverySortOption[] = ['lowest_investment', 'highest_profit', 'lowest_gap', 'shortest_payback', 'highest_roi'];
    const sortBy = allowedSorts.includes(rawSort) ? rawSort : 'lowest_investment';

    const result = businessService.discoverByBudget(availableCapital, { sortBy });
